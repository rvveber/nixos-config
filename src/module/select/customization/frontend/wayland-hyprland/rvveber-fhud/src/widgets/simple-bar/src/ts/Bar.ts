...
export default class Bar extends Astal.Window {
        ...
        GObject.registerClass(
            {
                ...
                InternalChildren: ["popover", "calendar", "traybox", "workspacebox"],
                Properties: {
                    ...string("clock"),
                    ...string("volume-icon"),
    ...
    declare current_workspace: string
    declare workspaceButtons: Gtk.Button[]
    declare updateWorkspaceHighlight: () => void;

    declare _popover: Gtk.Popover
    declare _calendar: Gtk.Calendar
    declare _traybox: Gtk.Box
    declare _workspacebox: Gtk.Box
    ...
    constructor() {
        ...
        this.current_workspace = hyprland.focused_workspace.name;

        hyprland.connect("client-moved", () => {
            this.current_workspace = hyprland.focused_workspace.name;
            this.updateWorkspaceHighlight();
        });

        this.workspaceButtons = [];

        for (let i = 1; i <= 10; i++) { // Adjusted to start from 1
            const button = new Gtk.Button({ label: `${i}` });
            const workspace = hyprland.get_workspace_by_name(`${i}`);
            ...
            if (workspace) {
                ...
                button.set_sensitive(false); // Make button unclickable if workspace is unused
            }

            button.add_css_class("workspace-button");
            button.set_name(`workspace-${i}`);
            this._workspacebox.append(button);
            this.workspaceButtons.push(button); // Maintain a reference to the button
        }

        this.updateWorkspaceHighlight = () => {
            const focusedWorkspace = hyprland.get_focused_workspace();
            log(focusedWorkspace);
            this.workspaceButtons.forEach((button, index) => {
                const workspaceName = button.get_label();
                if (focusedWorkspace && focusedWorkspace.name === workspaceName) {
                    button.grab_focus();
                } else {
